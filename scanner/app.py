...
본 시스템은 투자 권유가 아닙니다. 모든 투자 결정은 사용자 본인의 책임입니다.
"""
...
with st.sidebar:
    ...
    if _import_errors:
        with st.expander("모듈 임포트 경고", expanded=False):
    ...
    if not results:
        ...
        if selected:
            ...
            if fmt_match:
                ...
                st.divider()
                st.caption(fmt_match.get("disclaimer", "최종 매수 판단은 사용자 책임입니다."))
